...
ProjectionAlgos::ProjectionAlgos(const vector<TopoDS_Shape> &Inputs, const Base::Vector3d &Origin, const Base::Vector3d &Dir, const Base::Vector3d &xDir)
  : Inputs(Inputs), Origin(Origin), Direction(Dir), XDirection(xDir)
{
    execute();
}
...
void ProjectionAlgos::execute(void)
{
    ...
    gp_Ax2 transform(gp_Pnt(Origin.x, Origin.y, Origin.z),
		     gp_Dir(Direction.x,Direction.y,Direction.z),
		     gp_Dir(XDirection.x, XDirection.y, XDirection.z));
    // Base::Console().Message("Origin: (%f, %f, %f)\n", Origin.x, Origin.y, Origin.z);
    // Base::Console().Message("Dir: (%f, %f, %f)\n", Direction.x, Direction.y, Direction.z);
    ...
    shapes = new HLRBRep_HLRToShape( brep_hlr );
...
string ProjectionAlgos::getSVG2(int index,
				ExtractionType type, 
				vector<XmlAttributes> V_styles,
				vector<XmlAttributes> V0_styles, 
				double tolerance, 
				XmlAttributes V1_style, 
				XmlAttributes H_style, 
				XmlAttributes H0_style, 
				XmlAttributes H1_style)
{
    ...
    V  = build3dCurves(shapes->VCompound       (Inputs[index]));// hard edge visibly
    V1 = build3dCurves(shapes->Rg1LineVCompound(Inputs[index]));// Smoth edges visibly
    VN = build3dCurves(shapes->RgNLineVCompound(Inputs[index]));// contour edges visibly
    VO = build3dCurves(shapes->OutLineVCompound(Inputs[index]));// contours apparents visibly
    VI = build3dCurves(shapes->IsoLineVCompound(Inputs[index]));// isoparamtriques   visibly
    H  = build3dCurves(shapes->HCompound       (Inputs[index]));// hard edge       invisibly
    H1 = build3dCurves(shapes->Rg1LineHCompound(Inputs[index]));// Smoth edges  invisibly
    HN = build3dCurves(shapes->RgNLineHCompound(Inputs[index]));// contour edges invisibly
    HO = build3dCurves(shapes->OutLineHCompound(Inputs[index]));// contours apparents invisibly
    HI = build3dCurves(shapes->IsoLineHCompound(Inputs[index]));// isoparamtriques   invisibly
    
    if (!H.IsNull() && (type & WithHidden)) {
      Base::Console().Message("H---------------------------\n");
        H_style.insert({"stroke", "rgb(0, 0, 0)"});
        H_style.insert({"stroke-width", "0.15"});
        H_style.insert({"stroke-linecap", "butt"});
        H_style.insert({"stroke-linejoin", "miter"});
    ...
    if (!VO.IsNull()) {
      // Base::Console().Message("V0---------------------------\n");
        V0_styles[index].insert({"stroke", "rgb(0, 0, 0)"});
        V0_styles[index].insert({"stroke-width", "1.0"});
        V0_styles[index].insert({"stroke-linecap", "butt"});
        V0_styles[index].insert({"stroke-linejoin", "miter"});
        V0_styles[index].insert({"fill", "none"});
        V0_styles[index].insert({"transform", "scale(1,-1)"});
        BRepMesh_IncrementalMesh(VO,tolerance);
        result  << "<g";
        for (const auto& attribute : V0_styles[index])
            result << "   " << attribute.first << "=\"" 
                   << attribute.second << "\"\n";
        result << "  >" << endl
               << output.exportEdges(VO)
    ...
    if (!V.IsNull()) {
      // Base::Console().Message("V---------------------------\n");
        V_styles[index].insert({"stroke", "rgb(0, 0, 0)"});
        V_styles[index].insert({"stroke-width", "1.0"});
        V_styles[index].insert({"stroke-linecap", "round"});
        V_styles[index].insert({"stroke-linejoin", "round"});
        V_styles[index].insert({"fill", "none"});
        V_styles[index].insert({"transform", "scale(1,-1)"});
        BRepMesh_IncrementalMesh(V,tolerance);
        result  << "<g";
        for (const auto& attribute : V_styles[index])
            result << "   " << attribute.first << "=\"" 
                   << attribute.second << "\"\n";
        result << "  >" << endl
               << output.exportEdges(V)
