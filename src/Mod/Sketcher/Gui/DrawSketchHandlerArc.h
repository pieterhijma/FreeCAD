...
private:
    ...
    {
        return getToolHintsConcise();
    }

    std::list<Gui::InputHint> getToolHintsConcise() const
    {
        using enum Gui::InputHint::UserInput;
        using States = std::tuple<ConstructionMethod, SelectMode>;

        const auto switchHint = switchModeHint();
        States states = {constructionMethod(), state()};

        auto createHints = [&](const char* message) -> std::list<Gui::InputHint> {
            return {{QObject::tr(message), {MouseLeft}}, switchHint};
        };

        auto createSelectModeHints = [&](const char* first,
                                         const char* second,
                                         const char* third) -> Gui::HintTable<States, 1> {
            return {{.state = SelectMode::SeekFirst, .table = createHints(first)},
                    {.state = SelectMode::SeekSecond, .table = createHints(second)},
                    {.state = SelectMode::SeekThird, .table = createHints(third)}};
        };

        // clang-format off
        return Gui::lookupHints(
                states,
                Gui::HintTable<States, 0> {
                    {.state = ConstructionMethod::Center,
                     .table = createSelectModeHints("%1 pick arc center",
                                                    "%1 pick arc start point",
                                                    "%1 pick arc end point")},
                    {.state = ConstructionMethod::ThreeRim,
                     .table = createSelectModeHints("%1 pick first arc point",
                                                    "%1 pick second arc point",
                                                    "%1 pick third arc point")}});
        // clang-format on
    }

    std::list<Gui::InputHint> getToolHintsFull() const
    {
        using enum Gui::InputHint::UserInput;
        using States = std::tuple<ConstructionMethod, SelectMode>;
